		...
		switch ( $field['type'] ) {
			...
			case 'media':
				$upload_button = sprintf(
					'<input class="button rational-media-upload" type="button" value="Choose File">'
				);
				printf(
			...
			case 'checkbox':
				...
				foreach ( $field['choices'] as $value => $label ) {
					...
					if ( isset( $this->options[ $field['id'] ] ) ) {
						...
						if ( in_array( $value, $this->options[ $field['id'] ] ) ) {
							$checked = 'checked';
						} else {
							$checked = '';
						}
					...
					printf(
						'<label><input %s %s id="%s" name="%s[]" type="checkbox" title="%s" value="%s">&nbsp; %s</label>%s',
						$checked,																			// checked
						!empty( $field['class'] ) ? "class='{$field['class']}'" : '',						// class
						$field['id'],																		// id
						"{$page_key}[{$field['id']}]",														// name
